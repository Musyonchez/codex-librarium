...
interface StandaloneDashboardProps {
  bookType: 'singles' | 'novellas' | 'anthologies';
  bookTypeName: string;
  bookTypeNamePlural: string;
}
...
export default function StandaloneDashboard({
  ...
  const fetchData = async () => {
    setLoading(true);
    await Promise.all([fetchBooks(), fetchProgress()]);
    setLoading(false);
  };
  ...
  const fetchBooks = async () => {
    try {
      ...
      console.error(`Failed to fetch ${bookType}:`, error);
    }
  };

  const fetchProgress = async () => {
    try {
      const response = await fetch(`/api/reading/${bookType}`);
      if (!response.ok) {
        setProgress([]);
        return;
      }
  ...
  const stats = {
    ...
    unread: books.length - progress.length + progress.filter(p => p.status === 'unread').length,
  };
          ...
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
              <div className={`${styles.card} p-6`}>
                <div className={`${styles.textSecondary} text-sm mb-2`}>Total {bookTypeNamePlural}</div>
            ...
            <div className={`${styles.card} p-6`}>
              ...
              <p className={`${styles.textSecondary} mt-2 text-sm`}>
                You&apos;ve completed {stats.completed} out of {stats.total} {bookType}
              </p>
            </div>
