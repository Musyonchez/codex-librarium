...
export default function NovellasDashboardPage() {
  ...
  const fetchData = async () => {
    setLoading(true);
    await Promise.all([fetchNovellas(), fetchProgress()]);
    setLoading(false);
  };
  ...
  const stats = {
    total: novellas.length,
    completed: progress.filter(p => p.status === 'completed').length,
    reading: progress.filter(p => p.status === 'reading').length,
    unread: novellas.length - progress.length + progress.filter(p => p.status === 'unread').length,
  };
          ...
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
              <div className={`${styles.card} p-6`}>
                <div className={`${styles.textSecondary} text-sm mb-2`}>Total Novellas</div>
            ...
            <div className={`${styles.card} p-6`}>
              ...
              <p className={`${styles.textSecondary} mt-2 text-sm`}>
                You&apos;ve completed {stats.completed} out of {stats.total} novellas
              </p>
            </div>
