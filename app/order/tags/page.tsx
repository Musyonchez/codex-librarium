...
export default function OrderByTagsPage() {
  ...
  const booksByTag = useMemo(() => {
    ...
    const tagMap = new Map<string, BookWithSeries[]>();
      ...
      series.books.forEach((book) => {
        ...
        if (book.tags && book.tags.length > 0) {
          book.tags.forEach((tag) => {
            ...
            tagMap.get(tag)!.push(bookWithSeries);
          });
        }
      });
    });
  ...
  const filteredTags = useMemo(() => {
    if (!searchQuery) return Array.from(booksByTag.keys());

    const query = searchQuery.toLowerCase();
    return Array.from(booksByTag.keys()).filter((tag) =>
      tag.toLowerCase().includes(query)
    );
  }, [booksByTag, searchQuery]);

  return (
    <AppLayout requireAuth={true}>
      <div className="container mx-auto px-4 py-8">
        <div className="mb-8">
          <h1 className="text-4xl font-bold text-[#D4AF37] mb-2">Books by Tags</h1>
            ...
            {selectedTag ? (
              <div className="mb-6">
                ...
                <h2 className={`text-2xl font-bold ${styles.textGold} mb-4`}>
                  {selectedTag} ({booksByTag.get(selectedTag)?.length || 0} books)
                </h2>

                <div className="grid gap-2">
                  {booksByTag.get(selectedTag)?.map((book) => {
                ...
                {filteredTags.length === 0 ? (
                  ...
                  <div className="flex flex-wrap gap-3">
                    {filteredTags.map((tag) => {
                      const count = booksByTag.get(tag)?.length || 0;
                      return (
                        <button
                          key={tag}
